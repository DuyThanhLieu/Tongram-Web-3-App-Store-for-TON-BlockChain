...
test.describe("Tongram Actions 1", () => {
  test("Đăng nhập với Telegram và Thực hiện hành động", async ({ browser }) => {
    ...
    await test.step("Tìm kiếm một phần tử và nhấp vào nó", async () => {
      ...
      try {
        await page.waitForSelector("button.flex.w-full.items-center", {
          state: "visible",
          timeout: 20000,
        });
...
test.describe("Tongram Actions 1", () => {
  test("Đăng nhập với Telegram và thực hiện hành động", async ({ browser }) => {
    ...
    try {
      ...
      // Tìm kiếm một phần tử và nhấp vào nó
      const someElement = page.locator("button.flex.w-full.items-center");
